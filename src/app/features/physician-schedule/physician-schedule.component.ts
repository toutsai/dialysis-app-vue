...
export class PhysicianScheduleComponent implements OnInit, OnDestroy {
  ...
  async loadScheduleForDate(date: Date): Promise<void> {
    ...
    try {
      await this.fetchAllYearSchedules(year, month);
      const existingSchedule = await this.physicianSchedulesApi.fetchById(yearMonth) as any;
      this.scheduleData = this.generateBlankSchedule(year, month, this.availablePhysicians());
      this.consultationScheduleData = this.generateBlankConsultationSchedule(year, month, this.availablePhysicians());
      ...
      if (existingSchedule) {
        if (existingSchedule.schedule) {
          for (const day in existingSchedule.schedule) {
            if (this.scheduleData[day]) Object.assign(this.scheduleData[day], existingSchedule.schedule[day]);
          }
        }
        if (existingSchedule.consultationSchedule) {
          for (const day in existingSchedule.consultationSchedule) {
            if (this.consultationScheduleData[day]) Object.assign(this.consultationScheduleData[day], existingSchedule.consultationSchedule[day]);
          }
        }
        if (Array.isArray(existingSchedule.emergencyRecords)) this.emergencyRecords = existingSchedule.emergencyRecords;
        this.scheduleNotes = existingSchedule.notes || '';
        const dates = existingSchedule.specialDates || {};
        this.bloodDrawDate1 = dates.bloodDraw1 || '';
        this.bloodDrawDate2 = dates.bloodDraw2 || '';
        this.reportDate1 = dates.report1 || '';
        this.reportDate2 = dates.report2 || '';
        this.managedHolidays = existingSchedule.managedHolidays || [];
  ...
  async fetchPhysicians(): Promise<void> {
    try {
      const physicians = await this.usersApi.fetchAll([where('title', '==', '主治醫師')]) as any[];
  ...
  async saveAllChanges(): Promise<void> {
    ...
    const clinicUpdatePromises = this.availablePhysicians().map((doc: any) => {
      ...
      if ((doc.clinicHours || []).sort().join(',') !== [...newClinicHours].sort().join(',')) {
        return this.usersApi.update(doc.id, { clinicHours: newClinicHours });
      }
