...
export class WeeklyComponent implements OnInit, OnDestroy {
  ...
  async fetchArchivedSchedulesForWeek(dateStrings: string[]): Promise<any[]> {
    return await Promise.all(dateStrings.map(dateStr => this.archiveStore.fetchScheduleByDate(dateStr)));
  }
  ...
  async loadDataForWeek(): Promise<void> {
    ...
    try {
      ...
      const pastDates = datesToFetch.filter((d: string) => new Date(d) < today);
      const liveDates = datesToFetch.filter((d: string) => new Date(d) >= today);
      let fetchedRecords: any[] = [];
      if (liveDates.length > 0) {
        await this.patientStore.fetchPatientsIfNeeded();
        fetchedRecords.push(...await this.fetchLiveSchedulesForWeek(liveDates));
      }
      if (pastDates.length > 0) {
        fetchedRecords.push(...(await this.fetchArchivedSchedulesForWeek(pastDates)).filter(Boolean));
      }
      ...
      fetchedRecords.forEach((record: any) => newWeekRecords.set(record.date, record));
  ...
  runScheduleCheck(): void {
    ...
    for (const slotId in this.weekScheduleMap()) {
      const slotData = this.weekScheduleMap()[slotId];
      if (slotData?.patientId && !this.isDateInPast(parseInt(slotId.split('-').pop()!, 10))) scheduledIds.add(slotData.patientId);
    }
