...
export class StatsComponent implements OnInit, OnDestroy {
  ...
  get hasUnsavedChanges(): boolean {
    return this.hasUnsavedScheduleChanges || this.hasUnsavedTeamChanges;
  }

  get isPageLocked(): boolean {
    if (!this.auth.canEditSchedules()) return true;
  ...
  get sortedEarlyTeams(): string[] {
    const data = this.effectiveStatsData;
    if (!data || !data.early) return [];
    const teams = Object.keys(data.early);
  ...
  get sortedLateTeams(): string[] {
    const data = this.effectiveStatsData;
    if (!data || !data.late) return [];
    const teams = Object.keys(data.late);
    return teams.sort((a, b) => {
      if (a.includes('未分組')) return 1;
      if (b.includes('未分組')) return -1;
      if (a.includes('外圍')) return 1;
  ...
  get sortedLateTakeOffTeams(): string[] {
    const data = this.effectiveStatsData;
    if (!data || !data.lateTakeOff) return [];
    const teams = Object.keys(data.lateTakeOff);
    return teams.sort((a, b) => {
      if (a.includes('未分組')) return 1;
      if (b.includes('未分組')) return -1;
      if (a.includes('外圍')) return 1;
  ...
  get effectiveStatsData(): any {
    ...
    const messagesMap = this.taskStore.getPatientMessageTypesMapForDate(this.formatDate(this.currentDate));
  ...
  async loadData(date: Date): Promise<void> {
    ...
    try {
      ...
      if (patientIdsInSchedule.length > 0) {
        const ordersPromises = patientIdsInSchedule.map(async (patientId: string) => {
          const orders = await this.getEffectiveOrdersForDate(patientId, date);
          const patientInStore = this.patientMap.get(patientId);
          if (patientInStore) {
  ...
  async showInjectionList(teamData: any, shiftType: string | null = null): Promise<void> {
    ...
    if (shiftType && teamData[shiftType] && Array.isArray(teamData[shiftType].patients)) {
      teamData[shiftType].patients.forEach((p: any) => patientIdsToFetch.add(p.id));
    } else {
      for (const key in teamData) {
        if (teamData[key] && Array.isArray(teamData[key].patients)) {
          teamData[key].patients.forEach((p: any) => patientIdsToFetch.add(p.id));
        }
      }
    }
    ...
    try {
      ...
      this.dailyInjections = injectionsForGroup;
    } catch (error: any) {
