...
export class TaskStoreService implements OnDestroy {
  ...
  startRealtimeUpdates(uid: string): void {
    ...
    // -- Tasks assigned to this user (within retention window) --
    ...
    this.listeners.push(
      onSnapshot(myTasksQuery, (snapshot) => {
        ...
        console.error('[TaskStoreService] myTasks listener error:', error);
        this.isLoading.set(false);
      }),
    );
    ...
    // -- Feed messages (memos) within retention window --
