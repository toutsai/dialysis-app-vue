...
export class BedAssignmentDialogComponent implements OnChanges, OnInit, OnDestroy {
  ...
  get patientGroups(): Record<string, any[]> {
    ...
    if (this.assignmentMode === 'frequency' || this.assignmentMode === 'base') {
      const groups: Record<string, any[]> = { '未排床 - 急診': [], '未排床 - 住院': [], '未排床 - 門診': [] };
      const unassignedPatients = this.allPatients.filter(
        p => !p.isDeleted && !p.isDiscontinued && !this.localAssignedPatientIds.has(p.id) && p.freq
      );
      const filteredPatients = this.selectedFreq === 'all' ? unassignedPatients : unassignedPatients.filter(p => p.freq === this.selectedFreq);
